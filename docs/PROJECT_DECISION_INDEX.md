...
| ADR-005 | Selcafe read-only Phase 1 adapter | Done | **Accepted** — 2026-06-23 (Kerem approval). Full ADR text: read-only Selcafe Phase 1 via `CafeManagementAdapter`/`SelcafeAdapter`; bounded PII-free read surface + hard-exclusion list; SR-003-1—4 read-path controls. K-A1 decided (direct live SQL; replica/BigQuery deferred); K-A2 authorized (dedicated least-privilege read-only login); K-A4/K-A5 gate any future PII/cross-border expansion. Does NOT authorize Pod C; implementation blocked pending separately approved issues + (for PII/cross-border) `KVKK_LEGAL_BASIS.md` / `DATA_RETENTION_POLICY.md` / `CROSS_BORDER_TRANSFER_ASSESSMENT.md`. **v1.1 reconciliation (2026-06-24):** §4.1 read surface narrowed (`ayar` removed — open hours Adeks-native; `uyesinif` removed — data minimization); `urun` projection tightened (`tip` added, `menu` excluded; F&B filter = `tip=1 AND aktif=1`; category = `kod`-prefix convention only). Stub/deprecated classification notes added (§4.2). K-A3 → CLOSED. OQ-SC-NEW-001 → RESOLVED. |
...
| **K-20 — Selcafe Phase 1 product implications (PI-1/PI-2)** | `/docs/KEREM_DECISIONS.md` §20 | **[NEEDS KEREM APPROVAL — PR-gated].** Pod A proposal: PI-1 real-time station/session status is deferred to Phase 2 as an active product consumer except where K-21/KD-2 supersedes/subsumes K-20 PI-1 for customer-visible PC/session estimates inside the approved operating spine. Broader real-time station/session status, PC availability exposure, automatic reservation confirmation, and PC-status-dependent reservation automation remain deferred unless separately approved. PI-2 Phase-1 customer menu is Selcafe-sourced from `urun` / `menudetay` via vendor-neutral read models; category filtering remains blocked by the Selcafe category-source gap. Does not authorize Pod C. |
| **K-21 — Product Phase 1 operating-spine scope reconciliation** | `/docs/KEREM_DECISIONS.md` §21; `/docs/planning/SCOPE_RECONCILIATION_OPERATING_SPINE_ALIGNMENT_v0.1.md` | **Locked — Kerem-approved 2026-06-28.** Records K-OS-001…K-OS-007: addition-only guest ordering permitted; reliable Selcafe-linked PC estimates included; simplified first-slice customer F&B status with broader lifecycle internal/later; one simple PC + F&B coupon included; F&B + PC/session loyalty target after settlement; wallet payment/spending excluded from this operating spine only; 2% mismatch threshold, pilot pause triggers, and first-week admin checks approved. KD-1 records constrained Option B: product direction includes Selcafe-sourced active visit/bill/order-line visibility for the active `fiş` / visit, including cashier/staff-entered F&B items not submitted through Adeks PWA. Selcafe member identity/profile reads remain excluded. This does not authorize implementation and does not override ADR-005 by wording alone. ADR-005 read-surface expansion, KVKK/legal review, auditability, retention, and data-minimization review remain required before implementation. KD-2 records that K-OS-002 supersedes/subsumes K-20 PI-1 only for customer-visible PC/session estimates inside the operating spine. Phase 1 remains read-only toward Selcafe; Selcafe remains the settlement source of truth for this operating spine. Does not authorize Pod C, schema/API work, ADR drafting, direct Selcafe writes, wallet/payment implementation, or real data use. |
| **K-OS-008 — Adeks-owned discount + Selcafe reflection** | `/docs/KEREM_DECISIONS.md` §21 (K-OS-008) | **Locked — Kerem-approved 2026-06-28.** At PWA pilot, Selcafe's member-discount mechanism is retired; Adeks owns and calculates all discounts (coupon + loyalty); `adisyon.uye_indirim` is unused. Phase 1 reflects the discount into Selcafe via a cashier-entered fixed-format `kasaislem` record (`adisyon_no` + discount code + amount); Adeks does not write directly to Selcafe (human bridge). At settlement, Adeks reads the Selcafe `adisyon` sum + `kasaislem` discount, compares to its own discount-inclusive calculation, and green-lights the cashier within the 2% threshold (K-OS-007). Selcafe remains the settlement source of truth. Discount-ownership scope is platform-wide. Pre-implementation: the `kasaislem` reflection write and the `adisyon`/`kasaislem` reads remain under ADR-005 read-surface reconciliation + KVKK/legal/retention/data-minimization review (OQ-OS-006/OQ-OS-007). Does not authorize Pod C, schema/API work, ADR drafting, direct Selcafe writes, or real data use. |
