...
| OQ-OS-004 | Does the 2% mismatch warning apply to estimated total vs current Selcafe total, coupon-adjusted estimate, or both? | Kerem + Pod A + Pod B | partially resolved by K-OS-008; remaining: pre-settlement estimate-warning basis | Kerem, then Pod B | K-21 approves the 2% threshold as a pre-settlement warning threshold. K-OS-008 defines the settlement-time green-light basis as Adeks's discount-inclusive calculation vs the Selcafe `adisyon` sum reconciled with the `kasaislem` discount. The pre-settlement estimate-warning comparison basis remains open. Final settled amount must still come from Selcafe. |
...
| Former issue / dependency | Resolution | Remaining open dependencies / guardrails |
|---|---|---|
| Guest/addition flow vs account requirement | [RESOLVED - K-OS-009 / SL-6] Guests may link through the desk-side QR handshake, order F&B, and see the full live bill including itemized lines without an Adeks account. An Adeks account is required only for discounts, coupons, and points. Signup/login is offered alongside ordering, not as an ordering gate. | The live-bill read remains gated on ADR-005 read-surface expansion, KVKK/legal review, auditability, retention, and data-minimization review before implementation. |
