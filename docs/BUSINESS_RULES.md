...
| BR-OS-024 | Selcafe discount reflection (manual bridge) | For Phase 1, Adeks gives the cashier a predetermined fixed-format record (`adisyon_no` + discount code + discount amount); the cashier enters it into Selcafe `kasaislem` so the discount flows into Selcafe's final settlement. Adeks does not write directly to Selcafe; the cashier is the human bridge. The record must carry a non-identifying discount code only. | Confirmed by K-OS-008. [REQUIRES POD B REVIEW] for reverse-flow auditability and KVKK/data-minimization; fixed-format record spec is a Pod A item. ADR-005 reconciliation + KVKK/legal review required before implementation. Does not authorize direct Selcafe writes or Pod C. |
| BR-OS-025 | Settlement green-light check | At settlement, Adeks reads the Selcafe `adisyon` sum and the `kasaislem` discount, compares them against its own discount-inclusive calculation, and green-lights the cashier when within the 2% threshold (K-OS-007). Selcafe remains the settlement source of truth. | Confirmed by K-OS-008. [REQUIRES POD B REVIEW]. The `adisyon`/`kasaislem` reads require ADR-005 read-surface expansion, KVKK/legal review, auditability, retention, and data-minimization review before implementation. Does not authorize Pod C. |
