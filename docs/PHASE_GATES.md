...
| Operating-spine scope reconciliation | K-21 approved on 2026-06-28. Product Phase 1 spine is Selcafe-linked customer visibility and ordering. | Product documents must reflect K-21 before architecture/design or implementation-readiness work proceeds. Later Pod B review required for Selcafe read feasibility, matching, coupon/audit, loyalty/correction, KVKK/customer-data, cashier manual bridge, mismatch threshold, and pilot pause mechanics. This document does not authorize Pod C implementation. Phase 1 remains read-only toward Selcafe; Selcafe remains the settlement source of truth for this operating spine. |
