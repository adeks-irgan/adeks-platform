...
- The fixed-format record must carry a **non-identifying** dedicated Adeks `islem_tip` + pseudorandom one-time code (in `kasaislem.aciklama`) + amount (in `kasaislem.alacak`) — no `adisyon_no` and no Adeks customer/coupon/member identifier — to avoid pushing Adeks-linked personal data into Selcafe (data minimization). The bill key is not placed in Selcafe; Adeks holds the `code → adisyon_no` mapping internally.
